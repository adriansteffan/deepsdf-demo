...
def smooth_path(z_traj, n_frames):
    K = len(z_traj)
    out = []
...
def main():
    ...
    ap.add_argument('--out-dir', default='upscale_meshes')
    ap.add_argument('--n-frames', type=int, default=540)
    ...
    z_path = smooth_path(z_traj, args.n_frames)
    np.save(out_dir / 'z_path.npy', z_path)
    np.save(out_dir / 'z_traj_used.npy', z_traj)
    t0 = time.time()
    n_empty = 0
    ...
    print(f'{args.n_frames} in {time.time() - t0:.0f}s ({n_empty} empty)')
